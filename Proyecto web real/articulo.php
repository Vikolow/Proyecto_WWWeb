...
<body>
    ...
    <main class="mainCategorias">
        ...
        $Consulta_fav="SELECT * FROM favoritos WHERE id_usuario = {$_SESSION['id_usuario']} AND id_articulo = $id_art ;";
        ...
        if(isset($_REQUEST['activarFavorito'])){
            $añadir_favorito = "INSERT INTO favoritos (id_usuario, id_articulo) VALUES ( {$_SESSION['id_usuario']}, $id_art);";
            mysqli_query($conn,$añadir_favorito);
            header("Location: articulo.php");
        }

        //Función que elimina el favorito
        if(isset($_REQUEST['desActivarFavorito'])){
            $eliminar_favorito = "DELETE FROM favoritos WHERE id_usuario = {$_SESSION['id_usuario']} AND id_articulo = $id_art ;";
